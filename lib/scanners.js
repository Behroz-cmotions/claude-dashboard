...
function scanUsage(claudeDir) {
  ...
  try {
    const days = db.prepare(
      "SELECT date(timestamp) AS day, SUM(input_tokens + output_tokens) AS tokens, COUNT(*) AS turns " +
      "FROM turns WHERE timestamp >= datetime('now', '-14 days') GROUP BY day ORDER BY day"
    ).all();
    ...
    const tools = db.prepare(
      ...
      "WHERE tool_name IS NOT NULL AND tool_name != '' AND timestamp >= datetime('now', '-30 days') " +
...
function scanSkills(claudeDir) {
  ...
  if (fs.existsSync(skillsDir)) {
    for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
      ...
      let fm = {};
      try {
        fm = parseFrontmatter(fs.readFileSync(path.join(skillsDir, entry.name, 'SKILL.md'), 'utf8'));
      } catch {
