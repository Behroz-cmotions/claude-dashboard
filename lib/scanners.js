...
function scanHooks(claudeDir, projectPaths = []) {
  ...
  let hooks = flattenHooks(readJsonSafe(globalPath), 'globaal', globalPath);
  for (const p of projectPaths) {
...
function scanTokenUsage(claudeDir, cache = new Map(), titles = {}) {
  const dir = path.join(claudeDir, 'projects');
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    for (const f of files) {
      ...
      const tokens = cached.totals.input + cached.totals.output;
      const sessionId = f.replace(/\.jsonl$/, '');
