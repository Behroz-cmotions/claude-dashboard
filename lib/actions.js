...
function assertAllowed(target, roots) {
  if (!isAllowedPath(target, roots)) {
    throw new Error('pad ligt buiten de toegestane mappen: ' + target);
  }
...
function removeHook(settingsPath, event, groupIndex, hookIndex, roots) {
  ...
  if (!Array.isArray(groups)) throw new Error('event niet gevonden: ' + event);

  const group = groups[groupIndex];
  if (!group || !Array.isArray(group.hooks) || !group.hooks[hookIndex]) {
    throw new Error('hook niet gevonden op positie ' + groupIndex + '/' + hookIndex);
  }
...
function createFile(target, content, roots) {
  const p = assertAllowed(target, roots);
  if (fs.existsSync(p)) throw new Error('bestaat al: ' + p);
...
function addHook(settingsPath, event, matcher, command, roots) {
  const p = assertAllowed(settingsPath, roots);
  if (!event) throw new Error('event is verplicht');
  if (!command) throw new Error('commando is verplicht');
  let settings = {};
  if (fs.existsSync(p)) {
